...
//       <div className=" w-full bg-slate-50 text-white text-xs py-2.5 px-4 sm:px-8 lg:px-12 border-b border-slate-200 hidden md:block select-none">
...
const Header = () => {
  ...
  return (
    ...
    >
      {/* Top Bar - Professional Light Theme */}
      <div
        className={`w-full text-white text-xs py-2.5 px-4 sm:px-8 lg:px-12 select-none border-b border-transparent transition-all duration-300 ${isScrolled ? "hidden" : "hidden md:block"}`}
      >
        <div className="max-w-[1600px] mx-auto flex justify-between items-center">
          <div className="flex items-center gap-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <Button
                ...
                variant="theme"
                size="hd"
                className={`hidden md:inline-flex uppercase tracking-wider py-3 px-6 h-auto ${isScrolled ? "hover:text-text-primary" : "hover:text-white"} `}
              >
                Book a Consultation
              </Button>
              ...
              <button
                onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
                className={`lg:hidden p-2 rounded-none border focus:outline-none transition-colors ${isScrolled ? "text-slate-800 border-slate-200 hover:bg-slate-100" : "text-white border-white/20 hover:bg-white/10"}`}
                aria-label="Toggle menu"
              >
                {isMobileMenuOpen ? (
                  <svg
              ...
              if (link.name === "Sectors") {
                ...
                return (
                  <div key={link.name} className="flex flex-col">
                    ...
                    {isMobileSectorsOpen && (
                      <div className="bg-slate-50 pl-6 pr-3 py-2 flex flex-col gap-1 border-b border-slate-100 select-none">
                        {sectors.map((sec) => (
                          <Link
                            key={sec.href}
                            onClick={() => setIsMobileMenuOpen(false)}
